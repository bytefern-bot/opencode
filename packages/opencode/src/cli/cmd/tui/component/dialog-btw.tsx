...
export function DialogBtw(props: BtwHistoryItem) {
  ...
  async function refresh() {
    ...
    const visibleText = text.startsWith("<tool_") ? "" : text
    if (visibleText) setAnswer(visibleText)
    if (assistant?.info.time?.completed) {
      if (!visibleText) setAnswer("No text answer returned.")
      setLoading(false)
    }
  }
